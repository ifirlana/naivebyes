		...
		*/
		...
		if(mysql_query($sql)){
			...
			// Proses Probabilitas
				...
				$sql_katakunci = "select reg_artikel.*, katakunci_artikel.kata_kunci, detail_naivebyes.f_ci_plus_W, detail_naivebyes.f_wkjci_plus1
												from 
													reg_artikel left join katakunci_artikel 
														on reg_artikel.id_artikel = katakunci_artikel.id_artikel
													left join detail_naivebyes 
														on detail_naivebyes.id_artikel = katakunci_artikel.id_artikel_naivebyes
													group by reg_artikel.id, katakunci_artikel.id_kata, detail_naivebyes.id_artikel";
				$db_katakunci	=	mysql_query($sql_katakunci);
				while($rokk = mysql_fetch_array($db_katakunci)){
					
					//id_artikel
					if(!isset($rokk['id_artikel']) or empty($rokk['id_artikel'])){
							$id_artikel_kk = 0;
						}else{
							$id_artikel_kk = $rokk['id_artikel'];
							}
					
					//id_artikel
					if(!isset($rokk['kata_kunci']) or empty($rokk['kata_kunci'])){
							$kata_kunci_kk = 0;
						}else{
							$kata_kunci_kk = $rokk['kata_kunci'];
							}
					
					//id_artikel
					if(!isset($rokk['f_wkjci_plus1']) or empty($rokk['f_wkjci_plus1'])){
							$f_wkjci_plus1 = 1;
						}else{
							$f_wkjci_plus1 = $rokk['f_wkjci_plus1'];
							}
					
					$f_ci_plus_W = $fci + $rokk['total_kata'];
 					
					$TT_pci[] = array(
							"id_artikel" => $id_artikel_kk,
							"kata_kunci" => $kata_kunci_kk,
							"f_wkjci_plus1" => $f_wkjci_plus1,
							"f_ci_plus_W" => $f_ci_plus_W,
						);
					echo "load id_artikel : ".$id_artikel.", kata kunci, ".$kata_kunci_kk.", ".$f_wkjci_plus1.", ".$f_ci_plus_W."<br />";				
					}
				...
				while($row = mysql_fetch_array($db0)){
					...
					for($i=0;$i<count($TT_pci);$i++){ //proses perhitungan C*
						
						if($TT_pci[$i]['id_artikel'] == $id_artikel_p){
							
							$total_p = $total_p * $TT_pci[$i]['f_wkjci_plus1'] /$TT_pci[$i]['f_ci_plus_W'];
							echo $id_artikel_p." total_p".$total_p." * ".$TT_pci[$i]['f_wkjci_plus1']." / ".$TT_pci[$i]['f_ci_plus_W']."<br />";
							}
						}
