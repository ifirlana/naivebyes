	...
	if($_POST){
		...
		function check_perbandingan($kata, $kamus, $temp){ //inputan dari load_kamus
			...
			for($i=0; $i < count($kamus);$i++){
				...
				$array_string .= "check_perbandingan_".$i." { kamus : ".strtoupper($kamus[$i]['kata']).", kata : ".strtoupper($kata)."}, ";
		...
		for($i=0;$i<count($artikel);$i++){//di load idnya
			...
			while($row = mysql_fetch_array($result)){
				...
				//$array_string .= "inisiasi {teksAsli : '".$teksAsli."', artikel_sumber : '".$artikel_sumber."' , date_time : '".$date_time."'}, ";
				}
			...
			for($i=0;count($parts) > $i;$i++){
				...
				$array_string .= $i."_ARIFIN {".$check." : ".$parts[$i]."}, ";
			...
			foreach($temp_pencarian as $key => $value){
						...
						if($max['maximum'] < $temp_pencarian[$key]['hasil']){ //perbandingan
						 ...
						 $array_string .= "{ ".$key ." => ".  $temp_pencarian[$key]['hasil'] .", total_kata => ".$temp_pencarian[$key]['total']." }, "; //tampung untuk hasil result
						}
			$array_string .= "{ jumlah_kata => ".$total." } ";			
			
			///
			$sql_insert = "insert into artikel (id, artikel,artikel_sumber, date_time,id_kategori,result) values('','".$teksAsli."','".$artikel_sumber."','".$date_time."','".$temp_pencarian[$key]['id']."','".$array_string."')";
